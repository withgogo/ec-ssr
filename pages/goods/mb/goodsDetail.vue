<template>
    ...
    <div class="content-box bg">
        ...
        <div class="h5-banner-img-box">
            <video v-if="state.goodsMainImg.includes('mp4')" ref="videoPlayer" id="videoContextH5" class="h5-banner-img"
                controls show-play-btn show-center-play-btn object-fit="contain" :title="state.goodsData?.goodsName"
        ...
        <div class="flex bgfff align-items-center h5-banner-swiper-box" v-if="state.bannerList.length > 1">
            <div class="flex  align-items-center h5-banner-swiper">
                <div v-for="banner, index in state.bannerList" :key="index" class="h5-banner-swiper-item"
                    ...
                    <video v-if="banner.fullPath.includes('mp4')" autoplay muted playsinline @canplay="canplay($event)"
                        class="h5-banner-swiper-img" :title="state.goodsData?.goodsName">
            ...
            <svg-icon v-if="state.bannerList.length > 4" class="h5-banner-swiper-prev ponter" name="next"
                size="30"></svg-icon>
        </div>
        <!-- 价格 -->
        <div class="h5-goods-info bgfff h5-goods-info-price-box">
            ...
            <div v-if="state.isUserLvlUpgrade == 1"
                class="h5-member-price-question font12 flex  align-items-center justify-content-end"
    ...
    <div class="place"></div>
    <div v-if="state.goodsData?.goodsStatus !== 1" class="h5-footer-submit bgfff flex just-between align-items-center"
        style="z-index:100">
        <!-- 私密购的时候 不展示 -->
...
const allDataGoodsInfo = () => {
    ...
    let mainGoods = {
        ...
        memberFullSiteActivation: state.memberFullSiteActivation || undefined
    }
...
const toViewJump = (type) => {
    if (type == 'cart') {
        ...
        router.push({
            ...
            query: {
                id: state.advInfo?.group.id,
                isPrivate: state.advInfo?.group.is_private
            },
        });
    }
