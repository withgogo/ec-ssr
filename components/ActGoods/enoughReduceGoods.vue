<template>
                ...
                <div>
                    ...
                    <div class="flex  align-items-center just-between">
                        <div class="flex  align-items-center ">
                            <!--商品金额  -->
                            <memberTips v-if="goods.currentLevel && goods.currentLevel.rebate < 1">
                            </memberTips>
                            <div v-else :class="[layout == 'pc' ? 'pc-place-member' : 'h5-place-member']">
                            </div>
                            <!-- 会员价 -->
                            <div class="flex align-items-center price fontW act-goods-price"
                                :class="[layout == 'pc' ? 'pc-big-goods-price' : '']" v-if="goods.rebatePrice > 0">
                                <div>{{ $t('moneyUnit') }}</div>
                                <div>{{ goods.rebatePrice }}
                                </div>
                            </div>
                            <!-- 一般价格 -->
                            <div class="flex align-items-center price fontW act-goods-price"
                        ...
                        <div class="buy-btn buy pointer"
                            v-if="props.actInfo.erstatus !== -2 && props.actInfo.erstatus !== 2"
                            @click.stop="addBuyGoods(goods)">
                            {{ $t('button.buy') }}
                        </div>
                        <!-- 活动即将开始   -->
                        <div class="buy-btn buy pointer" v-if="props.actInfo.erstatus == 2">
                            {{ $t('groupAct.actAboutBegin') }}
                        </div>
                        <!-- 活动已结束  -->
                        <div class="buy-btn  activity-end " v-if="props.actInfo.erstatus == -2"
    ...
    <div v-else class="flex flex-wrap" :class="[layout !== 'pc' ? 'just-between' : '']">
        <div v-for="goods in props.goodsList" :key="goods.id" class="product-goods border"
            ...
            <div class="image-wrapper-goods">
                <img class="img" :style="{ width:layout == 'pc' ? '242px' : '171px' , height: layout == 'pc' ? '242px' : '171px' }"
                    mode="scaleToFill" :src="goods.goodsMainPhoto.fullPath" />
